...
class CustomCircleDrawable: Drawable {
    ...
    override fun draw(po: Canvas) {
        ...
        val ancho: Float = (po.width - 25).toFloat()
        val alto: Float = (po.height - 25).toFloat()
        val coordenadas = RectF(25.0F,  25.0F,  ancho, alto)
        po.drawArc(coordenadas,  0.0F,   360.0F,   false,   fondo)
        ...
        if (emociones.size != 0) {
            for (e in emociones) {
                ...
                po.drawArc(coordenadas!!, this.anguloInicio, this.anguloBarrido,   false,  seccion)
